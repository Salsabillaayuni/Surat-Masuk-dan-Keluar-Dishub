<?php
    ...
    elseif($_GET['act']=='edit'){
        ...
        if($_FILES['filesk']['name']==''){
            mysqli_query($conn, "UPDATE tb_suratkeluar SET no_suratkeluar='$no', no_agenda='$noag', tujuan_surat='$tujuan', judul_klasifikasi='$kla', isi_surat='$isi', tgl_surat='$tglsk', tgl_keluar='$tglk', ket_suratkeluar='$ket', file_suratkeluar='$filelama' WHERE id_suratkeluar='$id'");
            ...
            if(!in_array($ext,$ekstensi) ) {
                ...
                if($ukuran < 5044070){ //5Mb	
                    ...
                    $xx = $rand.'_'.$filename;
                    move_uploaded_file($_FILES['filesk']['tmp_name'], '../file/suratkeluar/'.$rand.'_'.$filename);
                    mysqli_query($conn, "UPDATE tb_suratkeluar SET no_suratkeluar='$no', no_agenda='$noag', tujuan_surat='$tujuan', judul_klasifikasi='$kla', isi_surat='$isi', tgl_surat='$tglsk', tgl_keluar='$tglk', ket_suratkeluar='$ket', file_suratkeluar='$xx' WHERE id_suratkeluar='$id'");
                    echo "<script language='javascript'>alert('Berhasil Mengedit data'); document.location.href='../suratkeluar.php';</script>";
                    } else {
                    unlink("../file/suratkeluar/$b[file_suratkeluar]");
                    $xx = $rand.'_'.$filename;
